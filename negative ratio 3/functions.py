...
"""====================================================================================================================
    ...
    |_.__/ \___|\__,_|_|  |___/
===================================================================================================================="""
def Load_GraphList(file_name:str):

    graph_list= load_graphs(file_name) # graph list, graph label
...
def shuffle_chunks(input, batch_num=10):
    ...
    #   dataset random slicing into n chunks 
...
"""
def Negative_Sampling(pos_sample, neg_samples, n_ratio):
    NS = list()
    # positive(48 hr) 先全留下來
    # 再根據 NS 決定是否刪除 negative samples
    # |train set| == P * 4
    if len(neg_samples)/len(pos_sample) < n_ratio:
        NS = deepcopy(neg_samples)
    else:
        NS = random.sample(neg_samples, len(pos_sample)*n_ratio)
    return NS

"""
    Process base graph , features, labels into formats and shape of new data graph
"""
def Data_Graph_Process(del_nodes, nodes_set, g, feature, active_matrix, label, hashtags):
    data_g = deepcopy(g)
    data_g = dgl.remove_nodes(data_g, del_nodes)
    # features process
    datag_f = feature[nodes_set]   # data graph feature matrix
    datag_f = torch.cat(
        (F.normalize(deepcopy(datag_f[:, 0:3]), dim= 1), 
        deepcopy(datag_f[:, 3:])), dim= 1)
    
    datag_h = hashtags[nodes_set]
    datag_f = torch.cat((datag_h, datag_f), dim=1)

    # labels and active process
    ...
    data_g = dgl.add_self_loop(g= data_g)
